...
# -------------------------
# Forecast Config and code
# -------------------------

class ForecastConfig:
    def __init__(self, forecast_steps=50, ema_alpha=0.7):
...
def ema_smooth(points, alpha):
    ...
    for i in range(1, len(points)):
        ...
        x = alpha * points[i][0] + (1 - alpha) * prev[0]
        y = alpha * points[i][1] + (1 - alpha) * prev[1]
...
def forecast_trajectory(points, steps):
    ...
    for i in range(1, steps + 1):
        ...
        future_point = p2 + velocity * i
        future.append(tuple(future_point.astype(int)))
        ...
        for cam_id, frame in [(1,frame1),(2,frame2)]:
            ...
            for obj,x,y,w,h in data[(cam_id,frame_id)]:
                ...
                if obj == 432:
                    ...
                    trajectory_history[obj].append((cx, cy))

                    # keep last 30 points
                    if len(trajectory_history[obj]) > 30:
                        trajectory_history[obj].pop(0)

                    smoothed = ema_smooth(
                        trajectory_history[obj],
                        forecast_config.ema_alpha
                    )

                    future = forecast_trajectory(
                        smoothed,
                        forecast_config.forecast_steps
                    )

                    trajectory_forecast[obj] = future
                ...
                if obj == 432:
                    ...
                    for p in trajectory_history[obj]:
                        cv2.circle(frame, (int(p[0]), int(p[1])), 2, (255,0,0), -1)

                    # draw forecast
                    if obj in trajectory_forecast:

                        for p in trajectory_forecast[obj]:
